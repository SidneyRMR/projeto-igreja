...
const Festas = (props) => {
    ...
    const alteraFesta = async (id_festa, nome_festa, data_inicio, data_termino) => {
        ...
        <div  >
            ...
            <Table className='tabela align-center' >
                ...
                <tbody>
                    {festas.map((festa, i) => {
            return (
                <tr key={festa.id_festa} className={i % 2 === 0 ? 'Par' : 'Impar'}>
                    {/* <td >{festa.id}</td> */}
                    <td>{festa.nome_festa}</td>
                    <td>{festa.data_inicio.slice(0,10)}</td>
                    <td>{festa.data_termino.slice(8,10)}</td>
                    <td className={+festa.data_termino.slice(8,10) !== 0 ? 'fechado' : 'aberto'}

                    >{+festa.data_termino.slice(8,10) !== 0 ? 'Encerrada' : 'Aberta'}</td>
                    <td>
                        {+festa.data_termino.slice(8,10) === 0 ?
                            <button className='botao w-100' onClick={() => alteraFesta(festa.id_festa, festa.nome_festa, 
                                festa.data_inicio, dataAtual())}>Encerrar Festa</button> : 
